...
seq() #так как все паарметры по умолчанию есть
...
########минизадание_2#######
#создать числовой вектор Big.data в котором будет последовательность чисел от 500 до 1000
...
A<-seq(3,60,by=3)
A<20 #каждый элемент вектора A сравнивается с 20, и если больше, то записывает вместо этого элемента FALSE,
#если меньше TRUE. То есть на выходе логический вектор длинна которого равна А, 
...
B=20 #это просто присвоит вектору значение (аналог <-)
...
C %in% K #факторный вектор сравнивается с сharacter вектором. Это норамально
...
#2.создать логический вектор Re, элемент которого равен TRUE если элемент последовательности чисел от 1 до 50 больше равно 30 
#и FALSE в противном случае

#3. создать логический вектор RE, элемент которого равен TRUE, если элемент последовательности чисел от 5 до 30, делящихся на
#10 равен 20 и FALSE в противном случае
#
#4. создать логический вектор Rt, элемент которого равен TRUE, если элемент последовательности ЧЕТНЫХ чисел от 1 до 500 
...
Re<-(seq(from=1, to=50)>30) #НЕПРАВИЛЬНО Скобка ничего не добавляет, только мешает
Re<-1:50>30 #ПРАВИЛЬНО

(mean(seq(from=1, to=10000)))-(mean(seq(from=5, to=500, by=5)))  ##НЕПРАВИЛЬНО, здесь сразу 3 лишние скобки
mean(1:10000)-mean(seq(from=5, to=500, by=5)) #ПРАВИЛЬНО
...
с<-c(a<50,b>80) #здесь объединяются два логических вектора a<50 и  b>80, не путать с a<50&b>80
...
b<-c(a<40 & !a %in% seq(1,40,3)) #НЕПРАВИЛЬНО
b<-a<40 & !a %in% seq(1,40,3)


######## Subsetting vector - вычленение из вектора его части. Два варианта#####################

#####Первый вариант - указание ВЕКТОРА порядковых элементов вектора, которые нужно выбрать
...
С<-letters[seq(2,length(letters),by=2)] #каждая вторая буква алфавита
...
A[50] <-"are you stupid, English alphabet is'nt so long"
...
#4. добавьте к вектору V в конце 3 новых элемента: 27, 56 и 98
...
#####Домашнее задание.#####
# Создайте  data frame: Mine из 5 рядов, в котором будут первые 5 наблюдений из ваших собственных данных
# количество и название колонок определите сами
