...
A #обратите внимания 2 в ковычках
class(A)
...
as.numeric(levels(C)) # так как levels(C) - это character
...
#создать вектор factor с именем ABC в котором будет 6 элементов, каждый из которых одна из 3 букв английского алфавита 
#(в произвольной последовательности), при этом установить order, где первая буква больше второй, а вторая больше третьей
...
B=20 #это просто присвоит вектору значения (аналог <-)
...
##############минизадание_3###########
#1.посчитать разность между двумя средними: 
